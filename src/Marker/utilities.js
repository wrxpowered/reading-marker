...
export function getParaRects(paraEle, paraId, offset) {
  ...
  for (let i = 0; i < wordsLength; i++) {
    var word = words[i];
    const wordsRects = word.getClientRects();
    const offsetTop = wordsRects[0].top - offset.y;
    const offsetLeft = wordsRects[0].left - offset.x;
    const offsetBottom = wordsRects[0].bottom - offset.y;
    const offsetRight = wordsRects[0].right - offset.x;
    const offsetWidth = wordsRects[0].width;
    const offsetHeight = wordsRects[0].height;
    const dataOffset = +word.getAttribute('data-offset');
    const dataLength = +word.getAttribute('data-length');

    if (
      offsetTop !== 0 || offsetLeft !== 0 || offsetWidth !== 0 || offsetHeight !== 0
    ) {
      if (wordsRects.length > 1) {
        // 单词断行
        for (let j = 0; j < wordsRects.length; j++) {
          const lineRect = wordsRects[j];
          const partItem = {
